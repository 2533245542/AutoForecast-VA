...
class AnalyzerPipeLineVarma:
    def __init__(self, pipelineVarma, lazy=False):
        """Inspect a pipelineVarma and produce convinient data structures for data science analysis."""
        # inputs
        self.pipelineVarma = pipelineVarma
        self.lazy = lazy

        # outputs
        self.dataImputationAndAveraging = pipelineVarma.dataImputationAndAveraging
        self.dataDelimitingByDayAndMedicalCenter = pipelineVarma.dataDelimitingByDayAndMedicalCenter
        self.dataCluseteringAndNormalization = pipelineVarma.dataCluseteringAndNormalization
        self.dataPreparationForFeatureSelection = pipelineVarma.dataPreparationForFeatureSelection
        self.featureSelectionGlobal = pipelineVarma.featureSelectionGlobal
        # self.day_level_clinic_level_varmaModel_dictionary = pipelineVarma.day_level_clinic_level_varmaModel_dictionary
        ...
        if not self.lazy:
            ...
            self.day_to_clinic_to_build_info_dataframe = self.get_day_to_clinic_to_build_info_dataframe(self.pipelineVarma)

            self.day_to_clinic_to_index_to_train_prediction_target_dataframe = self.get_day_to_clinic_to_index_to_train_prediction_target_dataframe(self.pipelineVarma)

            self.day_to_clinic_to_test_prediction_target_dataframe = self.get_day_to_clinic_to_test_prediction_target_dataframe(self.pipelineVarma)

            self.day_to_clinic_to_index_to_train_prediction_dataframe = self.get_day_to_clinic_to_index_to_train_prediction_dataframe(self.pipelineVarma)

            self.day_to_clinic_to_test_prediction_dataframe = self.get_day_to_clinic_to_test_prediction_dataframe(self.pipelineVarma)
    ...
    @staticmethod
    def get_day_to_clinic_to_build_info_dataframe(pipelineVarma):
        order_dictionary = AnalyzerPipeLineAutoForecastVA.transform_value_of_two_level_dictionary(pipelineVarma.day_level_clinic_level_arimaModel_dictionary, value_transform_function=lambda varmaModel: [str(order_tuple) for order_tuple in list(varmaModel.built_model_order_to_info_dictionary.keys())])
        order_dataframe = AnalyzerPipeLineAutoForecastVA.convert_nested_key_then_list_dictionary_to_others( first_level_second_level_list_dictionary=order_dictionary)
        order_dataframe['variable'] = 'AR_MA'

        aic_dictionary = AnalyzerPipeLineAutoForecastVA.transform_value_of_two_level_dictionary(pipelineVarma.day_level_clinic_level_arimaModel_dictionary, value_transform_function=lambda varmaModel: [info['aic'] for info in list(varmaModel.built_model_order_to_info_dictionary.values())])
        aic_dataframe = AnalyzerPipeLineAutoForecastVA.convert_nested_key_then_list_dictionary_to_others( first_level_second_level_list_dictionary=aic_dictionary)
        aic_dataframe['variable'] = 'aic'

        bic_dictionary = AnalyzerPipeLineAutoForecastVA.transform_value_of_two_level_dictionary(pipelineVarma.day_level_clinic_level_arimaModel_dictionary, value_transform_function=lambda varmaModel: [info['bic'] for info in list(varmaModel.built_model_order_to_info_dictionary.values())])
        bic_dataframe = AnalyzerPipeLineAutoForecastVA.convert_nested_key_then_list_dictionary_to_others( first_level_second_level_list_dictionary=bic_dictionary)
        bic_dataframe['variable'] = 'bic'

        concate_dataframe = pd.concat([order_dataframe, aic_dataframe, bic_dataframe], axis=0)
        reformatted_concate_dataframe = AnalyzerPipeLineAutoForecastVA.reformat_dataframe( input_dataframe=concate_dataframe, new_column_names=['day', 'clinic', 'index', 'value', 'variable'], name_of_index_column='day')

        return reformatted_concate_dataframe

    @staticmethod
    def get_day_to_clinic_to_index_to_train_prediction_target_dataframe(pipelineVarma):
        day_to_clinic_to_list_of_train_prediction_target_dictionary = AnalyzerPipeLineAutoForecastVA.transform_value_of_two_level_dictionary(first_level_second_level_item_dictionary=pipelineVarma.day_level_clinic_level_arimaModel_dictionary, value_transform_function=lambda varmaModel: varmaModel.train_dataframe.iloc[:, 0].tolist())
    ...
    @staticmethod
    def get_day_to_clinic_to_test_prediction_target_dataframe(pipelineVarma):
        dataframe = AnalyzerPipeLineAutoForecastVA.convert_nested_dictionary_to_others(first_level_second_level_item_dictionary=pipelineVarma.day_level_clinic_level_arimaModel_dictionary, item_converter=lambda varmaModel: varmaModel.test_dataframe.iloc[0, 0])
    ...
    @staticmethod
    def get_day_to_clinic_to_index_to_train_prediction_dataframe(pipelineVarma):
        day_to_clinic_to_list_of_train_prediction_dictionary = AnalyzerPipeLineAutoForecastVA.transform_value_of_two_level_dictionary(first_level_second_level_item_dictionary=pipelineVarma.day_level_clinic_level_arimaModel_dictionary, value_transform_function=lambda varmaModel: varmaModel.train_predictions.iloc[:, 0].tolist())
    ...
    @staticmethod
    def get_day_to_clinic_to_test_prediction_dataframe(pipelineVarma):
        dataframe = AnalyzerPipeLineAutoForecastVA.convert_nested_dictionary_to_others(first_level_second_level_item_dictionary=pipelineVarma.day_level_clinic_level_arimaModel_dictionary, item_converter=lambda varmaModel: varmaModel.test_predictions.iloc[0, 0])
    ...
    @staticmethod
    def save_analyzer_result_in_a_memory_efficient_way(pipelineVarma, file_name='data/memory_efficient_analyzer.dat'):
        """Remove the data heavy part of a pipeline, and store the rest."""
        analyzerPipeLineVarma = AnalyzerPipeLineVarma(pipelineVarma)
        analyzerPipeLineVarma.pipelineVarma = None
        analyzerPipeLineVarma.dataImputationAndAveraging = None
        analyzerPipeLineVarma.dataDelimitingByDayAndMedicalCenter = None
        analyzerPipeLineVarma.dataCluseteringAndNormalization = None
        analyzerPipeLineVarma.dataPreparationForFeatureSelection = None
        analyzerPipeLineVarma.featureSelectionGlobal = None
        analyzerPipeLineVarma.day_level_clinic_level_varma_dictionary = None

        VariableSaverAndLoader(save=True, list_of_variables_to_save=[analyzerPipeLineVarma], file_name=file_name)
