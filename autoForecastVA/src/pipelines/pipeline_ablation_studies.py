...
'''
`pipeline_no_clustering_has_feature_selection_and_has_one_tuning`
    First-level reduction:
    no clustering (every clinic is a cluster), feature selection and hyperparameter tuning for only the first day

`pipeline_no_clustering_no_feature_selection_and_has_one_tuning`
Second-level reduction:
no clustering (every clinic is a cluster), no feature selection (use all features) and hyperparameter tuning for only the first day
'''

class PipeLineTuningInterval():

    def __init__(self, medical_center_subset=None, time_period_subset=None, dataset_path='../../data/coviddata07292020.csv', number_of_days_for_data_averaging=3, max_number_of_cluster=10, number_of_days_to_predict_ahead=1, number_of_test_days_in_a_day_level_DataFrame=1, p_value_threshold=0.05, data_windowing_option=2, input_length=10, lstm_is_many_to_many=False, number_of_rolling_forecast_days=30, hyper_parameter_tuning_number_of_test_days_in_DataFrame=10, hyper_parameter_tuning_number_of_rolling_forecast_days=1, learning_rate_lower=0.00001, learning_rate_upper=0.1, number_of_hidden_dimensions_lower=5, number_of_hidden_dimensions_upper=50, dropout_rate_lower=0, dropout_rate_upper=0.7, hyper_parameter_space_seed=0, numpy_seed=0, CustomizedWorker=LSTMWorker, nameserver_run_id='example1', nameserver_address='127.0.0.1', nameserver_port=65300, min_budget=15, max_budget=160, n_iterations=4, tuning_frequency=1, neural_network_training_seed=0, loss_function=nn.MSELoss(), do_fine_tuning=True, trainable_parameter_name_list=None, do_clustering=True, do_general_model_training=True, do_normalization=True, do_feature_selection=True, batch_size_for_tuning=60, batch_size_for_general_model_building=60, batch_size_for_fine_tuning=60, generate_train_prediction_during_tuning=False, enable_data_caching_during_tuning=True, verbose=True, lazy=False):
        ...
        self.do_general_model_training = do_general_model_training # default to True
