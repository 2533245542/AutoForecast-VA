...
from autoForecastVA.src.benchmark.benchmark_parameter_util import VarmaParameterUtil
...
class BenchmarkVarmaPipeline():
    '''A benchmark for pipeline of running VARMA'''
    def __init__(self, list_of_input_dictionary=None, max_runner_cap=1, mapping_record_file_name='varma_mapping_md5_to_parameter_to_value.csv', run_multiprocessing=True, do_multiprocessing_dry_run=False, logger_name='VARMA_progress_logger', log_output_file_name='varma_multiprocessing_progress_logger.log', folder_to_save_results='results_varma', lazy=False):
        # inputs
        ...
        # outputs
        self.varmaParameterUtil = None
        self.list_of_parameter_to_modified_value_dictionary = None
        self.pool = None
        self.output = None
        if not self.lazy:
            self.varmaParameterUtil = VarmaParameterUtil(list_of_template_dictionary=self.list_of_input_dictionary)
            self.list_of_parameter_to_modified_value_dictionary = self.varmaParameterUtil.total_list_of_dictionary
    ...
    def do_a_run_of_pipeline(self, parameter_to_modified_value_dictionary, file_path_for_saving_result, do_multiprocessing_dry_run=False):
        ...
        if not do_multiprocessing_dry_run:
            pipelineVarma = PipelineVarma(**parameter_to_modified_value_dictionary)
            AnalyzerPipeLineVarma.save_analyzer_result_in_a_memory_efficient_way( pipelineVarma, file_name=file_path_for_saving_result)
