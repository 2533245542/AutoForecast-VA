...
class BenchMarkPipelineInterval():
    ...
    def start_multiprocessing(self):
        ...
        for parameter_to_modified_value_dictionary in self.list_of_parameter_to_modified_value_dictionary:
            md5_code = ComputeCharacterizationCodeAndMd5Code.get_class_MD5_code(target_class=PipeLineTuningInterval, parameter_to_modified_value_dictionary=parameter_to_modified_value_dictionary, store=True, md5_to_parameter_to_value_mapping_file_path=self.md5_to_parameter_to_value_mapping_file_path)
            file_path_for_saving_result = 'results/' + md5_code + '.dat'
    ...
    def generate_parameter_to_modified_value_dictionary_for_each_pipeline(base_nameserver_port=65100, max_tuning_interval=999999999, number_of_repeat_for_each_pipeline=5):
        ...
        for interval in [1, 5, 10, 30, max_tuning_interval]:
        ...
        # no clustering (has feature selection and tuning frequency = max)
        list_of_no_clustering_dict = []
        list_of_no_clustering_dict.append({'do_clustering': False, 'tuning_frequency': max_tuning_interval})

        # no clustering and no feature selection
        list_of_no_clustering_and_no_feature_selection_dict = []
        list_of_no_clustering_and_no_feature_selection_dict.append({'do_clustering': False, 'do_feature_selection': False, 'tuning_frequency': max_tuning_interval})

        total_list_of_dict = list_of_tuning_interval_dict + list_of_no_clustering_dict + list_of_no_clustering_and_no_feature_selection_dict
        ...
        for parameter_to_modified_value_dictionary in total_list_of_dict:
            repeated_total_list_of_dict += BenchMarkPipelineInterval.repeat_a_pipeline_by_seed(
